...
def pad_and_resize_images(folder_path):
    ...
    # Verifica qual atributo de redimensionamento usar (Pillow ≥10.0.0 vs anterior)
    try:
        ...
        for filename in files:
            ...
            try:
                with Image.open(file_path) as img:
                    ...
                    # Resize
                    resized_img = padded_img.resize((target_width, target_height), resample=resample_method)
...
def pdf_to_images(pdf_path, output_folder):
    ...
    print(f"[DEBUG] Starting conversion: {pdf_path} → {output_folder}")
    
    try:
        pdf_document = fitz.open(pdf_path)
        print(f"[DEBUG] PDF opened successfully with {len(pdf_document)} pages")
    except Exception as e:
    ...
    for page_number in range(len(pdf_document)):
        try:
            ...
            pix = page.get_pixmap(dpi=300)  # Alta qualidade
            output_path = os.path.join(output_folder, f"page_{page_number + 1}.png")
            pix.save(output_path)
            print(f"[DEBUG] Saved image: {output_path}")
        except Exception as e:
            print(f"[ERROR] Failed to process page {page_number + 1}: {e}")
...
def process_images(image_folder, output_folder):
    ...
    for indx in range(count_files_in_folder(image_folder, [".png", ".jpg", ".jpeg"])):
        filename = f'page_{indx + 1}.png'
        image_path = os.path.join(image_folder, filename)

        if indx == 0:
            # Simply copy the first page without splitting
            output_path = os.path.join(output_folder, f'image_{last_image_number}.png')
            img = cv2.imread(image_path)
            if img is not None:
                cv2.imwrite(output_path, img)
                print(f"Copied first page as-is: {filename} → image_{last_image_number}.png")
                last_image_number += 1
            else:
                print(f"Could not read image: {image_path}")
            continue

        # Split all other pages
        last_image_number = split_and_save_image(image_path, output_folder, last_image_number)
...
def run_full_preprocessing_pipeline(
    ...
    print("Converting PDF to images 2...")
...
def extract_image_number2(filename):
    """
    Extrai número da imagem a partir do nome do arquivo.
    Exemplo: 'res_image_10_Nobleza Virtuosa_sorted.txt' → '10'
    """
    match = re.search(r'res_image_(\d+)', filename)
    return int(match.group(1)) if match else None

def process_textfiles2(text_folder, bbox_folder, output_folder):
    # Listar arquivos de texto e bbox
    ...
    # Criar dicionário: número da imagem → nome do arquivo
    text_map = {extract_image_number2(f): f for f in text_files if extract_image_number2(f) is not None}
    bbox_map = {extract_image_number2(f): f for f in bbox_files if extract_image_number2(f) is not None}

    # Obter interseção dos números disponíveis
    common_keys = sorted(set(text_map.keys()) & set(bbox_map.keys()))

    if not common_keys:
        print("[ERRO] Nenhum pareamento entre texto e bounding boxes foi encontrado.")
        return

    print(f"[INFO] Pareando {len(common_keys)} arquivos texto/bbox.")

    os.makedirs(output_folder, exist_ok=True)

    for k in common_keys:
        text_path = os.path.join(text_folder, text_map[k])
        bbox_path = os.path.join(bbox_folder, bbox_map[k])
        output_path = os.path.join(output_folder, f"split_{k}.txt")

        with open(text_path, 'r', encoding='utf-8') as tf, open(bbox_path, 'r', encoding='utf-8') as bf:
            text_lines = tf.readlines()
            bbox_lines = bf.readlines()

            min_len = min(len(text_lines), len(bbox_lines))

            with open(output_path, 'w', encoding='utf-8') as out:
                for i in range(min_len):
                    out.write(f"{bbox_lines[i].strip()} ||| {text_lines[i].strip()}\n")

        print(f"[OK] Arquivo salvo: {output_path}")

    # Log de arquivos ignorados
    extra_text = set(text_map.keys()) - set(bbox_map.keys())
    extra_bbox = set(bbox_map.keys()) - set(text_map.keys())

    for k in extra_text:
        print(f"[WARN] Texto sem bbox correspondente: {text_map[k]}")
    for k in extra_bbox:
        print(f"[WARN] BBox sem texto correspondente: {bbox_map[k]}")
